...
/**
 * @param {{ community: Record<string, unknown>; to?: string; size?: 'default' | 'compact' }} props
 */
export function CommunityCard({ community, to, size = 'default' }) {
  const linkTo = to ?? `/community/${community.id}`;
  ...
  return (
    <Link to={linkTo} className='group block'>
      ...
      >
        <div
          className={cn(
            'relative w-full overflow-hidden bg-gradient-to-br from-[#FFF1E5] via-white to-primary-container/10',
            compact ? 'aspect-[5/3]' : 'aspect-[16/10]',
          )}
        >
          {community.cover_image ? (
            ...
            <div className='flex h-full items-center justify-center'>
              <Users className={cn(compact ? 'size-8' : 'size-10', 'text-[#FF8000]/30')} />
            </div>
          )}
          ...
          <div
            className={cn(
              'absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/75 via-black/35 to-transparent px-3',
              compact ? 'pb-2 pt-6' : 'pb-2.5 pt-8',
            )}
          >
            <h3
              className={cn(
        ...
        <CardContent className={cn('space-y-2', compact ? 'p-3' : 'p-3.5 md:p-4')}>
          ...
          <div className='flex flex-wrap items-center gap-2 text-xs font-medium'>
            <span className='inline-flex items-center gap-1 text-muted-foreground'>
              <Users className={cn(compact ? 'size-3' : 'size-3.5', 'text-[#FF8000]')} aria-hidden />
              {memberCount} anggota
            </span>
            <span
              className={cn(
