...
import { HomeEventListSkeleton } from '@/presentation/components/skeletons';
      ...
      try {
        ...
        if (Array.isArray(data)) items = data;
          ...
          items = Array.isArray(inner)
            ? inner
            : inner?.results || inner?.events || [];
        }
        const merged = items.map((event) => ({
          ...event,
          is_joined: Boolean(
            event.is_joined || activeEventIds.has(String(event.id)),
          ),
        }));
    ...
    <section className='space-y-4'>
      <div className='flex items-center justify-between gap-4'>
        <h2 className='font-display flex items-center gap-2 text-xl font-bold text-foreground md:text-2xl'>
          <Calendar
            className='size-6 shrink-0 text-[#FF8000] md:size-7'
            strokeWidth={2}
            aria-hidden
          />
          Event Mendatang
              ...
              >
                <Card className='rounded-2xl border border-border bg-card pr-4 pl-3 py-3 transition-all duration-300 group-hover:-translate-y-1 group-hover:shadow-md hover:bg-muted/30'>
                  <div className='flex items-center gap-3 md:gap-4'>
                    <div
                      className={`flex h-[60px] min-w-[60px] shrink-0 flex-col items-center justify-center rounded-xl md:h-[68px] md:min-w-[68px] ${dateBoxColors}`}
                    >
                      {event.event_date ? (
                        <>
                    ...
                    <div className='min-w-0 flex-1 space-y-1.5 py-0.5'>
                      ...
                      <div className='flex items-center gap-2 text-[12px]'>
                        <div
                          className={`inline-flex items-center rounded-md px-2 py-0.5 font-semibold ${tagColor}`}
                        >
